...
export default function CreateStorePage() {
  ...
  async function generate(regenerate = false) {
    ...
    const res = await fetch("/api/retailer/generate", {
      ...
      body: JSON.stringify({ imageUrl: location.origin + imageUrl, details, regenerate })
    });
    if (!res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card space-y-3">
        ...
        {imageUrl && <img src={imageUrl} alt="product" className="max-h-64 rounded" />}
        ...
        <div className="flex gap-3">
          <button disabled={!imageUrl || generating} onClick={() => generate(false)} className="rounded bg-indigo-600 px-4 py-2">{generating ? "Generating..." : "Generate store pack"}</button>
